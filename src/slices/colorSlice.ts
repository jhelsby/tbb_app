...
const setDarkModeFunc = (
  ...
  state.pageContrast = action.payload
    ? globalStyles.darkPage
    : globalStyles.lightPage;
  state.containerContrast = action.payload
...
const setColorsFunc = (
  state: TColorSliceState,
  action: PayloadAction<ColorActionPayload>,
): void => {
  const {startColor, startLightColor, endColor, endLightColor, length} =
    action.payload;
  const colors: string[] = [];
  const lightColors: string[] = [];
  for (let i = 0; i < length; i++) {
    const color: THSL = colorInterpolate(startColor, endColor, i / length);
    const lightColor: THSL = colorInterpolate(
      startLightColor,
      endLightColor,
      i / length,
    );
...
export const selectTextContrast = (state: RootState) =>
  state.color.textContrast;
export const selectPageContrast = (state: RootState) =>
  state.color.pageContrast;
export const selectContainerContrast = (state: RootState) =>
  state.color.containerContrast;
...
export const selectColor = createSelector(
  selectColors,
  getParams,
  (colors: string[], {index}: any): string => colors[index],
);
