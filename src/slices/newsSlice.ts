  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        ...
        (
          state: TNewsSliceState,
          action: PayloadAction<TNews[] | null | undefined>,
        ) => {
...
export const selectNewsById = createSelector(
  selectNews,
  getParams,
  (news: TNews[], {id}: any): TNews => {
    return news.find((article: TNews) => article.id === id) ?? defaultNews;
  },
);

export const selectNewsByIndex = createSelector(
  selectNews,
  getParams,
  (news: TNews[], {index}: any): TNews => {
