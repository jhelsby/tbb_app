...
const addReadingToStateFunc = (state: TReadingSliceState, action: PayloadAction<TReading>) => {
...
export const postReading = createAsyncThunk(
  ...
  async (index: number, thunkAPI) : Promise<{ docName: string, index: number } | null | undefined> => {
...
export const postAllReadings = createAsyncThunk(
  ...
  async (arg: any, thunkAPI) : Promise<{ docName: string, index: number }[] | null | undefined> => {
    const state: RootState = thunkAPI.getState() as RootState;
...
export const readingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(postReading.fulfilled, (state: TReadingSliceState, action: PayloadAction<{ docName: string, index: number } | null | undefined>) => {
      ...
      .addCase(postAllReadings.fulfilled, (state: TReadingSliceState, action: PayloadAction<{ docName: string, index: number }[] | null | undefined>) => {
        state.isLoading = false;
        state.hasError = false;
        if (action.payload) {
          action.payload.forEach((reading: { docName: string, index: number }) => {
      ...
      .addCase(fetchAllReadings.fulfilled, (state: TReadingSliceState, action: PayloadAction<TReading[] | null | undefined>) => {
        state.isLoading = false;
        state.hasError = false;
        if (action.payload) {
...
export const selectReadingById = createSelector(selectReadings, getParams, (readings: TReading[], { id }: any): TReading => {
  return readings.find((reading: TReading) => reading.id === id) ?? defaultReading;
});

export const selectReadingByIndex = createSelector(selectReadings, getParams, (readings: TReading[], { index }: any): TReading => {
